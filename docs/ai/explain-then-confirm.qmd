...
#### Exercise 1 `AI: Explainer`
```python
def gc_content(dna:
    return dna.count('G') / len(dna)
```

#### Exercise 2 `AI: Explainer`
```python
sequences = ['ATG', 'GCC', 'TAA'
print(sequences)
```

#### Exercise 3 `AI: Explainer`
```python
for base in 'ATGC':
print(base)
```

#### Exercise 4 `AI: Explainer`
```python
name = 'Preben'
print('Hello, ' name)
```
...
#### Exercise 5 `AI: Explainer`
```python
counts = {'A': 4, 'T': 3, 'G': 2}
print(counts['C'])
```

#### Exercise 6 `AI: Explainer`
```python
bases = ['A', 'T', 'G', 'C']
print(bases[4])
```

#### Exercise 7 `AI: Explainer`
```python
...
#### Exercise 9 `AI: Explainer`
```python
dna = 'ATGGCCTAA'
print(dna[::-1])
```

#### Exercise 10 `AI: Explainer`
```python
counts = {}
for base in 'ATGGCCA':
...
Find one error message or one piece of code from your own work this term, ideally from this week's exercises, that genuinely confused you at the time.
...
#### Exercise 15 `AI: Explainer`
Ask the assistant to explain the same error or code. Compare its explanation to the one you wrote yourself in Exercise 14. Confirm both against the actual running code. Which explanation was more precise, and which, if either, missed a detail that mattered?
