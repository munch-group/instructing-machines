...
## Checking from the machine, not just the browser {#sec-check-from-machine}
...
## The call can be right and the assumption still wrong {#sec-wrong-assumption}
...
## Four ways to be wrong, and the traceback each one leaves {#sec-invention-modes}
...
The third is the one the last section just walked through: a real function, called correctly, whose return value means something other than what you assumed. This is the quietest of the four, because it raises no exception at all. Nothing crashes, so nothing prompts you to go looking, which is exactly why the read-the-signature habit from @sec-check-from-machine has to run every time, not only when something visibly breaks.
