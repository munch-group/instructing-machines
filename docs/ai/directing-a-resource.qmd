...
You now have the two halves separately: reading a library's documentation as a set of contracts, and catching the assistant when it invents one that is not real. This clinic puts both halves to work on an actual small task, live, the way you will need to on this week's project and, eventually, on the finale. The task: given a DNA coding sequence, report whether its reverse complement, read as a coding sequence, translates cleanly to a protein with no internal stop codon.
