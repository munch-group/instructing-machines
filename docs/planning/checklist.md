...
- [ ] **Decide whether the badge is also an instruction.** A badge grants a licence; it does not tell the student what to do with the assistant. In the turtle chapters and the `ai/` notes the surrounding prose does that work. In the Python chapters the `AI: Explainer` badges added on error-message exercises are a licence only. If they should also prompt ("ask the assistant to explain this traceback, then run it and judge the explanation"), that sentence needs writing, exercise by exercise. Half of this is now answered: every badge links to a section saying what that rung permits, what it forbids and how the student proves the answer, so the general instruction is one click away everywhere. What remains is the exercise-specific half — the sentence that says what to ask *here*.
