# Project: Sequence trees {#sec-seqdist-project}
...
```txt
pixi run get seqdistproject
```

That puts the `seqdistproject` project folder straight into the `projects` folder inside your course folder, beside your notebooks.
...
You write your code in the file called `seqdistproject.py`. In the terminal, navigate to the project folder and run your code in the usual way:

```txt
python seqdistproject.py
```

To test the functions as you complete each one, you use `pytest` to run the project test suite in `test_seqdistproject.py` using this command:

```txt
pytest test_seqdistproject.py
```
...
That line makes the `log` (logarithm) builtin function from the `math` python library available to your program. Try to find its Python documentation to see how you use it.
