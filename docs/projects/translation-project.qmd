...
# Project: Translating ORFs {#sec-translation-project}
...
```txt
pixi run get translationproject
```

That puts the `translationproject` project folder straight into the `projects` folder inside your course folder, beside your notebooks.
...
You write your code in the file called `translationproject.py`. In the terminal, navigate to the project folder and run your code in the usual way:

```txt
python translationproject.py
```

To test the functions as you complete each one, you use `pytest` to run the project test suite in `test_translationproject.py` using this command:

```txt
pytest test_translationproject.py
```
