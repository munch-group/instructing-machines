# Project: Codon usage {#sec-codonbias-project}
...
```txt
pixi run get codonbiasproject
```

That puts the `codonbiasproject` project folder straight into the `projects` folder inside your course folder, beside your notebooks.
...
You write your code in the file called `codonbiasproject.py`. In the terminal, navigate to the project folder and run your code in the usual way:

```txt
python codonbiasproject.py
```

To test the functions as you complete each one, you use `pytest` to run the project test suite in `test_codonbiasproject.py` using this command:

```txt
pytest test_codonbiasproject.py
```
...
Now you have solved part of the task, what remains is to now use that function to normalize the codon counts of *each* amino acid in your grouped counts:
