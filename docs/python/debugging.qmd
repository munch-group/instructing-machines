...
By hand: `AATGATGC` contains `ATG` starting at position 1 and again at position 4, so the honest answer is 2, but a plausible-looking answer of 1 does not announce that it is wrong. There is no traceback here, so the method's first two steps have nothing to bite on, which is itself the lesson: when there is no error, the "failing line" is not a crash site, it is the line whose *assumption* is wrong. The hypothesis: `range(0, len(dna), 3)` only checks positions 0, 3, 6, stepping by three as if codons could not overlap, but the task explicitly asked for overlaps, so a start codon beginning at position 1 or 2 is never even looked at. Confirm it by printing the positions the loop actually checks: `list(range(0, 8, 3))` gives `[0, 3, 6]`, and position 1 is simply not among them. The fix is to step by one instead of by three:
