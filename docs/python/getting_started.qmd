...
We assume you already installed pixi (even though you cannot get it to work).

1. Quit VS Code completely first if it's running. 
...
3. Run this in the terminal and answer Y when prompted.

```txt
Set-ExecutionPolicy RemoteSigned -Scope CurrentUser

4. Delete any instructing-machines.zip files in your Downloads folder

5. Run this to unpack the course folder in a place Windows will not fight you

```txt
Expand-Archive -Path "$HOME\Downloads\instructing-machines.zip" -DestinationPath C:\im
```

6. Run this to navigate to the course-folder

```txt
cd C:\im\instructing-machines
```

7. Run this to build the environment in the course folder:
...
9. Then open C:\im\instructing-machines in VS Code using `File → Open Folder`.
