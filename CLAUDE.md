...
**Read first.** The authoritative course design now lives in `docs/planning/course-design.qmd` — it is both the plan and the second chapter of the book, and it holds the guiding principles, the week-by-week grid, the learning curve and cliffs, the AI-role ladder, the tool schedule, the workload shape, and the open decisions. The one-page student mental-model visual is `docs/planning/machine-map-poster.html`. The `weekplan.qmd` of the existing bioinformatics course, whose Python sequence and pace this course matches, is archived at `docs/planning/old_weekplan.md`. (The old `course-plan.md` has been folded into `course-design.qmd` and no longer exists.) Anything below is downstream of the design doc, with one exception: **`docs/_quarto.yml` is the authoritative week plan.** It declares the profile group (`default`, `ta`) and holds the book itself: the project settings, the `execute` block, the formats and every chapter list. Each `docs/_quarto-<profile>.yml` carries only that profile's overrides — its `output-dir`, its half of the cross-book margin link (an `other-links` entry each, repointed at the counterpart page by `docs/_includes/ta-link.html`), and for `default` the `post-render` chain that builds the student folder and then renders `ta`. Quarto merges the two files, so a chapter list could move into a profile file again; `scripts/quarto_profile.py` returns whichever file actually holds one, which is how the scripts survive that move. It is what the book actually renders, so where it and any prose document disagree about which chapter falls in which week, it wins. Each `- part: "Week N"` carries three annotations — `Python:` for what the week teaches, `AI:` for the ladder level it licenses, `Workload:` for what it costs a student — and the outstanding work on each chapter sits as a `CLAUDE:` comment under the chapter's own line.
...
- **`project-files/`** (repo root, **not** under `docs/`) — the runnable project code students download: each project dir has `<project>.py` (stub), `test_<project>.py` (the pytest suite), data files, `<project>_solution.py.encrypted`, and `solution_walkthrough.ipynb`. `hivproject/` also has `recap.py`, which is **not** a provided resource module despite its look: it opens with `from project1_solution import *`, a module that exists nowhere here, so it raises on import. It is scratch from the old course's project numbering, nothing imports it, no chapter mentions it, and `build_student_folder.py` holds it out of the hivproject zip. Delete it once you are sure nothing offline wants it. Projects are **not** in the day-one course folder: `scripts/build_student_folder.py` publishes one zip per project to `_book/project-files/`, and a student fetches the one they need with `im get <name>` (the same command that fetches a chapter notebook; both live in the **im-course-tools** package, installed into the student environment, not in the course folder) or the **Download project** button, which each project chapter carries both in its margin and in its opening prose. A project can therefore be edited up to the morning it is set. The zips carry neither `*_solution.py` nor `*_solution.py.encrypted` nor the walkthrough, so anything a student is not meant to have yet stays here.
...
- **The ladder is checked by machine.** `scripts/check-badge-order.py` walks the book's chapter list in render order, reads the badge under every `#### Exercise` heading (JSON-loading notebook cells, which a plain grep cannot see), and fails if any exercise licenses a role before the chapter that introduces it, if it finds no badges at all, or if a badge is still inline in its heading. It runs in `.github/workflows/quarto-publish.yml` before the render. Which config file that is comes from `scripts/quarto_profile.py`, shared by all four scripts that need the chapter list (this one, `todo.py`, `clean_notebooks.py`, `build_student_folder.py`): it resolves the profile the way Quarto does (`QUARTO_PROFILE`, else the `default:` in `docs/_quarto.yml`) and then returns whichever file actually lists chapters, so a script and the render it checks always read the same book even after the list moves between them. The publish workflow sets `QUARTO_PROFILE: default` once at job level for exactly that reason. Run `python3 scripts/check-badge-order.py --census` to print the per-role and per-week counts. When you move a note between weeks, update the `LADDER` map at the top of that script in the same commit.
...
`scripts/todo.py` is the pickup side. It reads the book's config itself and then
