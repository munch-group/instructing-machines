...
- **the-docs-are-the-test.qmd** — *Drafted (wk9).* The AI invents functions/endpoints; the real documentation is the test that fails them. `AI: Unreliable Narrator`.
...
- **reading-and-judging.md** — *Drafted (wk8).* Reading and judging code you did not write; the AI as Unreliable Narrator, the judging rubric.
...
- **delegate-the-full-problem.md** — *Drafted (wk11).* The full loop assembled once per piece on a task beyond your unaided reach; the Delegate role, the prompt journal. (Renamed from `delegating-a-whole-job`.)
